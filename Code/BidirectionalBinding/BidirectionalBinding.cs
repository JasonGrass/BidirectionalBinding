...
namespace Jgrass.Tools.Binding
{
    ...
    public class BidirectionalBinding
    {
        ...
        internal BidirectionalBinding(object bindableSource, string sourcePath, object bindableTarget, string targetPath)
        {
        ...
        internal void InnerSetBinding()
        {
            ...
            if (!BindableSource.IsAlive || !BindableTarget.IsAlive)
            {
                return;
            }

            ((INotifyPropertyChanged) BindableSource.Target).PropertyChanged += Source_OnPropertyChanged;
            ((INotifyPropertyChanged) BindableTarget.Target).PropertyChanged += Target_OnPropertyChanged;
            ...
            switch (InitMode)
            {
                ...
                case BindingInitMode.SourceToTarget:
                    ...
                    var sourceValue = ValueConverter == null ?
                        ...
                        GetValue(BindableSource.Target, SourcePath, ValueConverter.Convert, _targetPropertyType, null, out success1);

                    if (success1)
                    {
                        SetValue(BindableTarget.Target, TargetPath, sourceValue);
                    }
                ...
                case BindingInitMode.TargetToSource:
                    ...
                    var targetValue = ValueConverter == null
                        ...
                        : GetValue(BindableTarget.Target, TargetPath, ValueConverter.ConvertBack, _sourcePropertyType, null, out success2);

                    if (success2)
                    {
                        SetValue(BindableSource.Target, SourcePath, targetValue);
                    }
                    break;
            }
        ...
        private void Source_OnPropertyChanged(object sender, PropertyChangedEventArgs propertyChangedEventArgs)
        {
            ...
            var value = ValueConverter == null ?
                GetValue(BindableSource.Target, SourcePath) :
                GetValue(BindableSource.Target, SourcePath, ValueConverter.Convert, _targetPropertyType, null, out success);
            if (success)
            {
        ...
        private void Target_OnPropertyChanged(object sender, PropertyChangedEventArgs propertyChangedEventArgs)
        {
            ...
            var value = ValueConverter == null
                ? GetValue(BindableTarget.Target, TargetPath)
                : GetValue(BindableTarget.Target, TargetPath, ValueConverter.ConvertBack, _sourcePropertyType, null, out success);
            if (success)
            {
        ...
        private void VerifyProperty()
        {
            ...
            {
                throw new InvalidOperationException("Can Not binding different type of two properties without implementing a converter.");
            }
        }
